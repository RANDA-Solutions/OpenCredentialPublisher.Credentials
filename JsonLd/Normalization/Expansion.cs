...
namespace OpenCredentialPublisher.Credentials.JsonLd.Normalization
{
    internal class Expansion
    {
                                                ...
                                                Func<object, JToken> expansionMap = null)
        {
            ...
            // mapped
            if (rval != null && !(options?.KeepFreeFloatingNodes ?? false) && !insideList &&
                ...
                if (ctx == null)
                {
                    ...
                    if (!overrideProtected && activeCtx.Protected.Any())
                    {
                        ...
                        else if (protectedMode == "warn")
                        {
                            ...
                            foreach (var valuePair  in oldActiveCtx.Protected)
                            {
                                var _protected = valuePair.Value;
                                var term =  valuePair.Key;
#else
