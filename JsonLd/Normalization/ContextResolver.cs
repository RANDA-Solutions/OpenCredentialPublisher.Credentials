...
using OpenCredentialPublisher.Credentials.JsonLd.Normalization.Exceptions;
using System;
...
namespace OpenCredentialPublisher.Credentials.JsonLd.Normalization
{
...
#endif
        protected Dictionary<ExpandContext, ExpandContext> Cache
        {
            get;
#if NET6_0
            init;
            ...
            set;
#endif
        } = new Dictionary<ExpandContext, ExpandContext>();
    ...
    {
        protected static readonly ConcurrentDictionary<string, (string, string, DateTime)> documentCache = new ConcurrentDictionary<string, (string, string, DateTime)>();
        ...
        protected virtual async Task<(JObject, string)> FetchContext(ExpandContext activeCtx, string url, HashSet<object> cycles)
        {
            ...
            try
            {
                ...
                using (var reader = new JsonTextReader(new StringReader(docBody)) { DateParseHandling = DateParseHandling.None })
                    context = JToken.Load(reader);
        ...
        protected virtual async Task<(string, string)> LoadDocument(string url, List<string> redirects = null)
        {
            ...
            // handle redirect
            if ((alternate != null || (int)res.StatusCode >= 300 && (int)res.StatusCode < 400) &&
                location != null)
            {
        ...
        protected ResolvedContext CacheResolvedContext(string key, ResolvedContext resolved, string tag = null)
        {
            ...
            if (tag != null)
            {
                if (!SharedCache.TryGetValue(key, out var tagMap))
                {
                    tagMap = new Dictionary<string, ResolvedContext>();
